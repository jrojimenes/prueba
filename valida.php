...
$idu = mysql_result($consulta, 0, 'idusuario');
    ...
    if ($activo == 'desactivado') {
        echo "<div class='alert-danger'><center>Usuario esta desactivado, consulte a su administrador</center></div>";
                        ...
                        {
                            $sql1="Select * from usuario where usuario='$user' and contrasena='$psw'";
                            $consulta1=mysql_query($sql1) or die(mysql_error());
                            $idu = mysql_result($consulta1, 0, 'idusuario');
                            $activo = mysql_result($consulta1, 0, 'status');
